...
impl MultipartSigned {
    ...
    pub fn sign(ctx: &impl IsA<CryptoContext>, entity: &impl IsA<Object>, userid: &str) -> Result<Option<MultipartSigned>, glib::Error> {
...
impl<O: IsA<MultipartSigned>> MultipartSignedExt for O {
    fn verify(&self, flags: VerifyFlags) -> Result<Option<SignatureList>, glib::Error> {
        unsafe {
            let mut error = ptr::null_mut();
            let ret = ffi::g_mime_multipart_signed_verify(self.as_ref().to_glib_none().0, flags.into_glib(), &mut error);
            if error.is_null() { Ok(from_glib_full(ret)) } else { Err(from_glib_full(error)) }
        }
    }
}
